...
class ParallelMatrixMultiplier {
    ...
    /* returns matrix product C = AB */
    public int[][] computeProduct() throws ExecutionException, InterruptedException {
        int numWorkers = Runtime.getRuntime().availableProcessors();
        ExecutorService pool = Executors.newFixedThreadPool(numWorkers);
        Future<int[][]>[] futures = new Future[numWorkers]; // Array of futures that hold a 2D int array
        int chunkSize = (int) Math.ceil((double) numRowsA / numWorkers); // number of rows each worker will process
    ...
    // Handle calculation of a subset of rows
