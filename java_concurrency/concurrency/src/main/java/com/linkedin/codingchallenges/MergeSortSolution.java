...
class ParallelMergeSorter {
    ...
    /* worker that gets called recursively. RecursiveAction does not return a result, in this case
    * it is not needed, since the sort the array in place. */
    private class ParallelWorker extends RecursiveAction {
        ...
        protected void compute() {
            if (left < right) {
                ...
                invokeAll(leftWorker, rightWorker); // Execute both forked processes
                merge(left, mid, right); // merge the two sorted halves
            }
        }
    }
